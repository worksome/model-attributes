...
[<img src="https://github-ads.s3.eu-central-1.amazonaws.com/model-attributes.jpg?t=1" width="419px" />](https://spatie.be/github-ad-click/model-attributes)
...
```bash
php artisan vendor:publish --tag="model-attributes-migrations"
php artisan migrate
```
...
```bash
php artisan vendor:publish --tag="model-attributes-config"
```
...
```bash
php artisan vendor:publish --tag="model-attributes-views"
```

## Usage

```php
$modelAttributes = new Worksome\ModelAttributes();
echo $modelAttributes->echoPhrase('Hello, Worksome!');
```
